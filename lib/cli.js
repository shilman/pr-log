...
function getValidLabels(prLogConfig) {
    if (prLogConfig) {
        const { validLabels, skipLabels } = prLogConfig;
        const result = validLabels ? new Map(validLabels) : new Map(defaultValidLabels);
        if (skipLabels) {
            skipLabels.forEach((label) => result.set(label, label));
        }
        return result;
    }
...
export default function createCliAgent(dependencies) {
    ...
    async function generateChangelog(options, githubRepo, validLabels, newVersionNumber) {
        ...
        const pullRequests = await getMergedPullRequests(githubRepo, validLabels, options.cherryPick, options.sincePrerelease);
